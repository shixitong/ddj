...
(function($) {
				...
				if (flashInstalled) {
					...
					swfuploadify.queueData = {
						...
						uploadQueue : [], // The files currently to be
											// uploaded
						errorMsg : '文件加入队列失败：'/*'Some files were not added to the queue:'*/
					};
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg = '文件加入队列失败：'/*'Some files were not added to the queue:'*/;
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('名称为 "'   /*The file named*/  
							+ file.name
							+ '" 的文件已经存在队列中\n您想替换掉对列中的文件吗？ ' /*is already in the queue.\nDo you want to replace the existing item in the queue?*/);
					if (!replaceQueueItem) {
