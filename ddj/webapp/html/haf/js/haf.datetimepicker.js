...
(function($) {
	...
	DateTimePicker.prototype = {
		...
		init: function(element, options) {
            ...
            this.maxDate = options.maxDate ?
                (typeof(options.maxDate)=="string" ? this.parseDate(options.maxDate) : options.maxDate ) : false;
            this.minDate = options.minDate ?
                (typeof(options.minDate)=="string" ? this.parseDate(options.minDate) : options.minDate ) : false;

            if(!this.pickTime){
                if(this.maxDate) this.maxDate = new Date(this.maxDate.getFullYear(),this.maxDate.getMonth(),this.maxDate.getDate(),23,59,59,999);
                if(this.minDate) this.minDate = new Date(this.minDate.getFullYear(),this.minDate.getMonth(),this.minDate.getDate(),0,0,0,0);
            }

            this.checkValue = typeof options.checkValue=="function" ?  options.checkValue : false;
		...
		set: function() {
            if(this.minDate && this.minDate>this._date){
                alert("选择日期不能小于"+this.formatDate(this.minDate));
                //this.hide();
                return;
            }
            if(this.maxDate && this.maxDate<this._date){
                alert("选择日期不能大于"+this.formatDate(this.maxDate));
                //this.hide();
                return;
            }

            //checkValue
            if(this.checkValue && !this.checkValue(this._date,this.formatDate(this._date))){
                return;
            }
		...
		setStartDate: function(date) {
			if (date instanceof Date) {
				...
				this.startDate = new new Date(date);
		...
		setEndDate: function(date) {
			if (date instanceof Date) {
				...
				this.endDate = new new Date(date);
		...
		fillHours: function() {
			...
			if (this.options.pick12HourFormat) {
				...
				for (var i = 0; i < 5; i += 1) {
					html += "<tr>";
					for (var j = 0; j < 4; j += 1) {
                        if(current > 0 && current<17 && current % 4==0){
                            current++;
                        }
						var c = current.toString();
						html += '<td class="hour">' + padLeft(c, 2, "0") + "</td>";
				...
				if (!target.is(".disabled")) {
					...
					case "td":
						if (target.is(".day")) {
							...
							this._date = new Date(year, month, day, this._date.getHours(), this._date.getMinutes(), this._date.getSeconds(), this._date.getMilliseconds());
		...
		doAction: function(e) {
			e.stopPropagation();
			e.preventDefault();
			if (!this._date) this._date = new Date(1970, 0, 0, 0, 0, 0, 0);
		...
		formatDate: function(d) {
			...
			function(match) {
				...
				if (methodName === "getYear") rv = rv + 1900 - 2e3;
		...
		_finishParsingDate: function(parsed) {
			...
			if (parsed.Year) year = 2e3 + parsed.Year;
			if (!year) year = 1970;
		...
		_attachDatePickerEvents: function() {
            ...
            this.widget.on("click", ".picker-time-bot .btn:eq(0)" ,function(){
                self.input.val("");
                self.hide();
            });
            this.widget.on("click", ".picker-time-bot .btn:eq(1)" ,function(){
                self.hide(true);
            });
			/*if (this.pickDate && this.pickTime) {
	...
	function getTemplate(timeIcon, pickDate, pickTime, is12Hours, showSeconds, collapse) {
		if (pickDate && pickTime) {
			return '<div class="bootstrap-datetimepicker-widget dropdown-menu">' +
                ...
                '<td class="picker-line"><div></div></td>' +
                '<td class="picker-time"><div class="picker-time-tit"><i class="glyphicon glyphicon-time"></i></div>' +
                '<div class="timepicker">' + TPGlobal.getTemplate(is12Hours, showSeconds) +
                "</div>" +
                '<div class="picker-time-bot"><a class="btn btn-default">清除</a>&nbsp;<a class="btn btn-default">确定</a></div></td>' +
                "</tr></table>" +
                "</div>"
		} else if (pickTime) {
			return '<div class="bootstrap-datetimepicker-widget dropdown-menu">' +
                "<table><tr>" +
                '<td class="picker-time only-time">' +
                '<div class="timepicker">' + TPGlobal.getTemplate(is12Hours, showSeconds) +
                "</div>" +
                '<div class="picker-time-bot"><a class="btn btn-default">清除</a>&nbsp;<a class="btn btn-default">确定</a></div></td>' +
                "</tr></table>" +
	...
	DPGlobal.template = '<div class="datepicker-days">' +
        '<table class="table-condensed">' + DPGlobal.headTemplate + "<tbody></tbody>" +
        "</table>" +
        "</div>" +
        '<div class="datepicker-months">' +
        '<table class="table-condensed">' + DPGlobal.headTemplate + DPGlobal.contTemplate +
        "</table>" +
        "</div>" +
    ...
    //
    $.DateTimePicker = {
        isLeapYear: function(year){return DPGlobal.isLeapYear(year);},
        getDaysInMonth: function(year, month){return DPGlobal.getDaysInMonth(year, month);},
        parseDate: function(date, format){return DPGlobal.parseDate(date, DPGlobal.parseFormat(format));},
        formatDate: function(date, format){return DPGlobal.formatDate(date, DPGlobal.parseFormat(format));}
    }
})(window.jQuery);