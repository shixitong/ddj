...
(function($) { // Picker object
    var Datepicker = function(element, options) {
        ...
        this.component = this.element.is('.date-input-group') ? this.element.find('.input-group') : false;
        this.input = this.component ? this.element.find('input') : false;
        if (this.isInput) {
            this.element.on({
        ...
        this.scroll = (options.scroll != undefined ? options.scroll: false);
        this.weekEnd = this.weekStart == 0 ? 6 : this.weekStart - 1;
        this.maxDate = options.maxDate ?
            (typeof(options.maxDate)=="string" ? DPGlobal.parseDate(options.maxDate,this.format) : options.maxDate ) : false;
        this.minDate = options.minDate ?
            (typeof(options.minDate)=="string" ? DPGlobal.parseDate(options.minDate,this.format) : options.minDate ) : false;
        this.fillDow();
        this.fillMonths();
        this.update();
        this.showMode();
    ...
    Datepicker.prototype = {
        ...
        place: function() {
            ...
            var offset = this.input ? this.input.offset() : this.element.offset();
            this.picker.css({
        ...
        update: function() {
            var date = this.component ? this.element.find('input').val() : this.element.val();
                    ...
                    case 'td':
                        if (target.is('.day')) {
                            ...
                            if(this.minDate && this.minDate>date){
                                alert("选择日期不能小于"+DPGlobal.formatDate(this.minDate, this.format));
                                break;
                            }
                            if(this.maxDate && this.maxDate<date){
                                alert("选择日期不能大于"+DPGlobal.formatDate(this.maxDate, this.format));
                                break;
                            }
